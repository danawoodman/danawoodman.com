<script lang="ts">
	const links = [
		{
			href: 'mailto:dana@danawoodman.com',
			title: 'Shoot me an email',
			icon: 'email',
		},
		{
			href: 'https://github.com/danawoodman',
			title: 'Check out my repos on Github',
			icon: 'github',
		},
		{
			href: 'https://www.linkedin.com/in/danawoodman',
			title: 'Follow me on LinkedIn',
			icon: 'linkedin',
		},
		{ href: '/sitemap.xml', title: 'View site map', icon: 'sitemap' },
	]
</script>

<footer class="text-center my-20 clear-both" id="page-footer">
	<nav class="my-8 flex items-center justify-center">
		{#each links as link}
			<a
				href={link.href}
				target="_blank"
				class="inline-block text-gray-400 hover:text-gray-900 transition-colors mx-3"
				title={link.title}
			>
				{#if link.icon === 'email'}
					<svg
	...
	<p class="text-gray-400 my-4">
		<small>Site built with SvelteKit, MDsveX, and TailwindCSS.</small>
	</p>
</footer>
